...
#         # # work2 = Process(target=self.displayclass.start_analysis())
#         #
#         #
#         # work1.start()
#         # work2.start()
...
#         # # pool.apply(self.overlayClass.RunSetWindow(),args=())
#         # # pool.apply(self.displayclass.analysize(),args=())
#         # pool.map(self.overlayClass.RunSetWindow())
...
class MainWindow(QMainWindow, ui_file):
    ...
    def analysisStartBtn_clicked(self):
        ...
        self.processmanager.overlay.RunOverlay()
        self.processmanager.display.start_analysis(); self.RunTossWindowSize()

    # 분석 종료 버튼 클릭 이벤트 함수
    def analysisEndBtn_clicked(self):
        print("End Button Clicked")
        self.processmanager.overlay.StopOverlay()
        self.processmanager.display.stop_analysis()


    def RunTossWindowSize(self):
        self.timer = QtCore.QTimer(self)
        self.timer.timeout.connect(self.processmanager.TossWindowSize)
        self.timer.start()
