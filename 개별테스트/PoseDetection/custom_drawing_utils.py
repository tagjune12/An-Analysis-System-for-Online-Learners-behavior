...
def draw_landmarks(
    ...
    for key in idx_to_coordinates.keys():
        landmark_px = idx_to_coordinates[key]
        if key > 0 and key < 7:
            continue
        if key == 0:  # 코는 하늘색
            # print(f'Key: {key}  Value: {idx_to_coordinates[key]}')
            ...
            cv2.circle(image, landmark_px, landmark_drawing_spec.circle_radius,
                       (255, 255, 0), 10)  # (B, G, R)
        elif key <= 10:  # 입, 귀는 빨간색
            cv2.circle(image, landmark_px, landmark_drawing_spec.circle_radius,
                       (0, 0, 255), landmark_drawing_spec.thickness)  # (B, G, R)
        elif key < 13:  # 어깨는 파란색
            cv2.circle(image, landmark_px, landmark_drawing_spec.circle_radius,
                       (255, 0, 0), 10)  # (B, G, R)
        elif key == 13:  # 왼쪽 팔꿈치는 노란색
            cv2.circle(image, landmark_px, landmark_drawing_spec.circle_radius,
                       (0, 255, 255), 10)  # (B, G, R)
        elif key == 14:  # 오른쪽 팔꿈치는 보라색
            cv2.circle(image, landmark_px, landmark_drawing_spec.circle_radius,
                       (255, 0, 255), 10)  # (B, G, R)
        elif key == 15 or key == 16:  # 양쪽 손목
            cv2.circle(image, landmark_px, landmark_drawing_spec.circle_radius,
                       (100, 100, 100), 10)  # (B, G, R)
        else:
            continue
...
def plot_landmarks(landmark_list: landmark_pb2.NormalizedLandmarkList,
    ...
    for idx, landmark in enumerate(landmark_list.landmark):
        ...
        ax.scatter3D(
            ...
            ys=[landmark.x],
            zs=[-landmark.y],
            color=_normalize_color(landmark_drawing_spec.color[::-1]),  # 빨간색
        ...
        if idx == 11 or idx == 12:
            ax.scatter3D(
                ...
                ys=[landmark.x],
                zs=[-landmark.y],
                color=_normalize_color([0, 0, 255]),  # (R, G, B)
                linewidth=landmark_drawing_spec.thickness)
        ...
        if idx == 0:
            ax.scatter3D(
                ...
                ys=[landmark.x],
                zs=[-landmark.y],
                color=_normalize_color([0, 255, 0]),
                linewidth=landmark_drawing_spec.thickness)
        ...
        if idx == 13:
            ax.scatter3D(
                ...
                ys=[landmark.x],
                zs=[-landmark.y],
                color=_normalize_color([255, 255, 0]),
                linewidth=landmark_drawing_spec.thickness)
        ...
        if idx == 14:
            ax.scatter3D(
                ...
                ys=[landmark.x],
                zs=[-landmark.y],
                color=_normalize_color([0, 255, 255]),
                linewidth=landmark_drawing_spec.thickness)
